 ...
 */
async function captureScreen(captureScreenInfo, appRoot, db, chromiumBrowser) {
	...
	const openPageTimeoutInMilliSec = config.get('Screenshots.phantomOpenPageTimeoutInSec') * 30000,
		userAgentString = config.get('Screenshots.userAgent'),
		screenshotExpirationHours = config.get('Screenshots.screenshotExpirationHours'),
	...
	try {
		...
		// Start actual web crawling using puppeteer
		puppeteerPage = await chromiumBrowser.newPage();
		// setup crawler
		...
		/*
		// ycx!!!!!!
		const button = await puppeteerPage.$('div.closePopup');
		if (button) {
			await button.click();
		}
		*/
		...
		await puppeteerPage.waitFor((multiPaneNativeCount) => {
			console.log('000000 window.devicePixelRatio: ' + window.devicePixelRatio);
			// ycx!!!!!!
			deviceRatio = window.devicePixelRatio;
			// ycx!!!!!!
			// Found something new, the image src url from cloudinary has w_0 in chromium, but working image src urls should have w_90
			// the width gets decided in js-dist->main.js->_getTransformUrlFromCanonical function
			var cliquesImages = [];
			...
			return true;
		}, {timeout: 6000}, multiPaneNativeCount);
		...
		logger.error(`****** CATCH ERROR: ${JSON.stringify(err)} and devicePixelRatio: ${deviceRatio} and screen websiteUrl: ${captureScreenInfo.websiteUrl}, pid: ${captureScreenInfo.pid}, crgId: ${captureScreenInfo.crgId}`);
        if (puppeteerPage) {
			await puppeteerPage.close();
        }
		if (err.logLevel) {
