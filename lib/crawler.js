...
// So we need to make sure phantom instance quit before the crawling promise returns.
// If it is hanging there, we manually kill the hanging phantom process
function makeSurePhantomExit(phantomPID) {
    var allPhantomPIDS = getAllPhantomPIDs();
    if (allPhantomPIDS.indexOf(phantomPID) === -1) {
        return;
    } else {
        // by the time `makeSurePhantomExit` func is called, phantom has finished its job,
        // if it still exist, we could safely kill it now
...
function captureScreen(captureScreenInfo, appRoot, db) {
    ...
    .then(function(page) {
        ...
        .then(function() {
            ...
            return makeSurePhantomExit(phantomPID);
        });
    })
    .catch(function(err) {
        var errorString;
